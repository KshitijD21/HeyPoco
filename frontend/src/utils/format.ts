...
/**
 * Get a human-friendly confidence label from a cosine similarity score.
 * Distance: 0.0 = identical, 2.0 = opposite. Lower = stronger match.
 */
export function getSimilarityLabel(similarity: number): {
  label: string
  color: string
} {
  if (similarity < 0.3) return { label: 'Strong match', color: 'text-emerald-600' }
  if (similarity < 0.6) return { label: 'Related', color: 'text-yellow-600' }
  return { label: 'Weak match', color: 'text-black/30' }
}
